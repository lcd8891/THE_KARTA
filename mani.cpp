...
bool ззакрыть{false},анимка{false};
...
ничего получить_коорды(const sf::Event &event){
    if(event.type == sf::Event::KeyReleased && event.key.code == sf::Keyboard::Space){
        ...
        sf::Vector2i ь = {
            (int)((ф.x - 372) / 536.f * 100), 
            (int)((ф.y - 55) / 536.f * 100)
        };
...
ничего сцена(){
    if(зе_сцена.кадр() || кнопка_нажата("stop")){
        стринги a,b,тмп;
        да_нет ы = анимка;
        зе_карта.загрузить_место(место,a,b,анимка);
        тмп = b + L"\n" + a;
        стр = обработка(тмп,40);
        текст_по_символу = 0;
        забыть_кнопки();
        добавить_кнопку("back to map",{540,600},{100,30});
        if(анимка)добавить_кнопку("play",{1230,670},{100,100});
        включить_хит(музыки[1]);
...
ничего открыть_просмотр(){
    ...
    if(кнопка_нажата("play")){
        зе_карта.загрузить_сцену_из_места();
        выключить_хит();
        забыть_кнопки();
        добавить_кнопку("stop",{25,25},{50,50});
        указатель = сцена;
    }
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Space)){
        текст_по_символу=стр.size();
    }
    ...
    if(анимка)спрайт(5,{1230,670},true);
}
            ...
            ещё_указатель = [](const sf::Event &event) {
                if(event.type == sf::Event::KeyPressed){
                    if(event.key.code == sf::Keyboard::Left && место > 0){
                        ...
                        текст_по_символу = 0;
                        if(анимка && !ы){
                            добавить_кнопку("play",{1230,670},{100,100});
                        }else if(!анимка && ы){
                            убрать_кнопку("play");
                        }
